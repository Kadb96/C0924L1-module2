...
public class Run {
    public static void main(String[] args) {
        ...
        System.out.println("Triangle is successfully created with 3 sides: " + a + b + c);
    }

    public static void isTriangleIsIllegal(float a, float b, float c) throws IllegalTriangleException {
        if (a <= 0 || b <= 0 || c <= 0) {
            throw new IllegalTriangleException("All sides must > 0");
        } else if (a + b <= c || a + c <= b || b + c <= a) {
            throw new IllegalTriangleException("Can't make triable with sides: " + a + ", " + b + ", " + c);
        }
    }
}
